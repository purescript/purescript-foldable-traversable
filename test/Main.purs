...
import Data.Foldable (class Foldable, find, findMap, fold, head, last, indexl, indexr, foldMap, foldMapDefaultL, foldMapDefaultR, foldl, foldlDefault, foldr, foldrDefault, length, maximum, maximumBy, minimum, minimumBy, null, surroundMap)
...
main = do
  ...
  log "Test head"
  assert $ head [] == (Nothing :: Maybe Unit)
  assert $ head [1, 5, 10] == Just 1

  log "Test last"
  assert $ last [] == (Nothing :: Maybe Unit)
  assert $ last [1, 5, 10] == Just 10
