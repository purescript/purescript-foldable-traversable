...
import Data.Bifoldable (class Bifoldable, bfoldlWithIndex, bfoldrWithIndex, bfoldMapWithIndex, bfoldrWithIndexDefault, bfoldlWithIndexDefault, bfoldMapWithIndexDefaultR, bfoldMapWithIndexDefaultL)
import Data.Bifunctor (class Bifunctor, bimap)
import Data.Bitraversable (class Bitraversable, bisequenceDefault, btraverseWithIndex, bisequence, btraverseWithIndexDefault)
...
-- test whether foldable laws hold, using foldMap and foldMapWithIndex
testFoldableWithIndexLawsOn
  :: forall f i a m n e 
...
testFoldableWithIndexLawsOn c f g = do
  ...
  -- These follow from the above laws, but they test whether foldlWithIndexDefault and
  -- foldrWithIndexDefault have been specified correctly.
...
instance bifoldableIOr :: Bifoldable IOr where
  bfoldrWithIndex l r u (Both fst snd) = l fst (r snd u)
  bfoldrWithIndex l r u (Fst fst)      = l fst u
  bfoldrWithIndex l r u (Snd snd)      = r snd u

  bfoldlWithIndex l r u (Both fst snd) = r (l u fst) snd
  bfoldlWithIndex l r u (Fst fst)      = l u fst
  bfoldlWithIndex l r u (Snd snd)      = r u snd

  bfoldMapWithIndex l r (Both fst snd) = l fst <> r snd
  bfoldMapWithIndex l r (Fst fst)      = l fst
  bfoldMapWithIndex l r (Snd snd)      = r snd
...
instance bitraversableIOr :: Bitraversable IOr where
  btraverseWithIndex f g (Both fst snd) = Both <$> f fst <*> g snd
  btraverseWithIndex f g (Fst fst)      = Fst <$> f fst
  btraverseWithIndex f g (Snd snd)      = Snd <$> g snd
...
testBifoldableIOrWith lift fst snd u = do
  assert $ bfoldrWithIndex (+) (*) u (lift $ Both fst snd) == fst + (snd * u)
  assert $ bfoldrWithIndex (+) (*) u (lift $ Fst fst)      == fst + u
  assert $ bfoldrWithIndex (+) (*) u (lift $ Snd snd)      == snd * u

  assert $ bfoldlWithIndex (+) (*) u (lift $ Both fst snd) == (u + fst) * snd
  assert $ bfoldlWithIndex (+) (*) u (lift $ Fst fst)      == u + fst
  assert $ bfoldlWithIndex (+) (*) u (lift $ Snd snd)      == u * snd

  assert $ bfoldMapWithIndex Additive Additive (lift $ Both fst snd) == Additive (fst + snd)
  assert $ bfoldMapWithIndex Additive Additive (lift $ Fst fst)      == Additive fst
  assert $ bfoldMapWithIndex Additive Additive (lift $ Snd snd)      == Additive snd
...
testBitraversableIOrWith lift = do
  ...
  assert $ btraverseWithIndex Just Just (lift (Both true false))     == just (Both true false)
  assert $ btraverseWithIndex Just Just (lift (Fst true))            == just (Fst true  :: IOr Boolean Boolean)
  assert $ btraverseWithIndex Just Just (lift (Snd false))           == just (Snd false :: IOr Boolean Boolean)


-- structures for testing default `Bifoldable` implementations

newtype BfoldMapWithIndexDefaultL l r = BFML (IOr l r)
newtype BfoldMapWithIndexDefaultR l r = BFMR (IOr l r)
newtype BfoldlWithIndexDefault    l r = BFLD (IOr l r)
newtype BfoldrWithIndexDefault    l r = BFRD (IOr l r)

instance eqBFML :: (Eq l, Eq r) => Eq (BfoldMapWithIndexDefaultL l r) where eq (BFML l) (BFML r) = l == r
instance eqBFMR :: (Eq l, Eq r) => Eq (BfoldMapWithIndexDefaultR l r) where eq (BFMR l) (BFMR r) = l == r
instance eqBFLD :: (Eq l, Eq r) => Eq (BfoldlWithIndexDefault l r)    where eq (BFLD l) (BFLD r) = l == r
instance eqBFRD :: (Eq l, Eq r) => Eq (BfoldrWithIndexDefault l r)    where eq (BFRD l) (BFRD r) = l == r

instance bifoldableBFML :: Bifoldable BfoldMapWithIndexDefaultL where
  bfoldMapWithIndex f g m        = bfoldMapWithIndexDefaultL f g m
  bfoldrWithIndex f g u (BFML m) = bfoldrWithIndex f g u m
  bfoldlWithIndex f g u (BFML m) = bfoldlWithIndex f g u m

instance bifoldableBFMR :: Bifoldable BfoldMapWithIndexDefaultR where
  bfoldMapWithIndex f g m        = bfoldMapWithIndexDefaultR f g m
  bfoldrWithIndex f g u (BFMR m) = bfoldrWithIndex f g u m
  bfoldlWithIndex f g u (BFMR m) = bfoldlWithIndex f g u m

instance bifoldableBFLD :: Bifoldable BfoldlWithIndexDefault where
  bfoldMapWithIndex f g (BFLD m) = bfoldMapWithIndex f g m
  bfoldrWithIndex f g u (BFLD m) = bfoldrWithIndex f g u m
  bfoldlWithIndex f g u m        = bfoldlWithIndexDefault f g u m

instance bifoldableBFRD :: Bifoldable BfoldrWithIndexDefault where
  bfoldMapWithIndex f g (BFRD m) = bfoldMapWithIndex f g m
  bfoldrWithIndex f g u m        = bfoldrWithIndexDefault f g u m
  bfoldlWithIndex f g u (BFRD m) = bfoldlWithIndex f g u m


-- structures for testing default `Bitraversable` implementations

newtype BtraverseWithIndexDefault l r = BTD (IOr l r)
newtype BisequenceDefault l r = BSD (IOr l r)

instance eqBTD :: (Eq l, Eq r) => Eq (BtraverseWithIndexDefault l r) where eq (BTD l) (BTD r) = l == r
instance eqBSD :: (Eq l, Eq r) => Eq (BisequenceDefault l r) where eq (BSD l) (BSD r) = l == r

instance bifunctorBTD :: Bifunctor BtraverseWithIndexDefault where bimap f g (BTD m) = BTD (bimap f g m)
instance bifunctorBSD :: Bifunctor BisequenceDefault where bimap f g (BSD m) = BSD (bimap f g m)

instance bifoldableBTD :: Bifoldable BtraverseWithIndexDefault where
  bfoldMapWithIndex f g (BTD m) = bfoldMapWithIndex f g m
  bfoldrWithIndex f g u (BTD m) = bfoldrWithIndex f g u m
  bfoldlWithIndex f g u (BTD m) = bfoldlWithIndex f g u m

instance bifoldableBSD :: Bifoldable BisequenceDefault where
  bfoldMapWithIndex f g (BSD m) = bfoldMapWithIndex f g m
  bfoldrWithIndex f g u (BSD m) = bfoldrWithIndex f g u m
  bfoldlWithIndex f g u (BSD m) = bfoldlWithIndex f g u m

instance bitraversableBTD :: Bitraversable BtraverseWithIndexDefault where
  btraverseWithIndex f g     = btraverseWithIndexDefault f g
  bisequence (BTD m) = map BTD (bisequence m)

instance bitraversableBSD :: Bitraversable BisequenceDefault where
  btraverseWithIndex f g (BSD m) = map BSD (btraverseWithIndex f g m)
